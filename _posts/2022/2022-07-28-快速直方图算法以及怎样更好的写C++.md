...
for (unsigned i = 0; i < num_target_fp; i++) {
  auto x = target_fp_vec[i];
  size_t now_frame = x.second;
  for (auto& xx : anchor_matched[i]) {
    int16_t frame = xx.second - now_frame;
    int16_t vote_frame = (frame - (-375));
...
事实上，一个好的问题抽象至关重要。开始的时候，因为问题抽象不正确会导致抽象问题的性能提升不能等价到原问题的性能提升。
...
int thread_start = thread_i * num_target_fp / thread_n;
  ...
  for (unsigned i = thread_start; i < thread_end; ++i) {
    ...
    for (auto& xx : (anchor_matched)[i]) {
      int16_t frame = xx.second - now_frame;
      int16_t vote_frame = (frame - (-375));
      if (vote_frame < 0) continue;
      if (vote_frame >= int(col_size)) continue;
      size_t name_idx = xx.first;
...
v2b
```text
int v2b(std::vector<uint32_t> target_fp_vec,
        std::vector<std::vector<uint32_t>> anchor_matched) {
  std::vector<unsigned short> vec_vote_pool;
  vec_vote_pool.resize(row_size * col_size);

  spdlog::info("Start voting");
  int start = utils::GetCurrentTime();
  for (unsigned i = 0; i < num_target_fp; i++) {
    auto x = target_fp_vec[i];
    size_t hash_key = x >> 12;
    size_t now_frame = x & 4095;
    // spdlog::info("V2b:{} {}", hash_key, now_frame);

    for (auto& xx : anchor_matched[i]) {
      int16_t frame = (xx & 8191) - now_frame;
      int16_t vote_frame = (frame - (-375));
      if (vote_frame < 0) continue;
      if (vote_frame >= int(col_size)) continue;
      size_t name_idx = (xx >> 13);
      // spdlog::info("V2b match:{} {} {} {}", xx, name_idx, vote_frame,
      //              name_idx * col_size + vote_frame);
      vec_vote_pool[name_idx * col_size + vote_frame] += 1;
    }
  }
  spdlog::info("Finish voting");
  int profile = utils::GetCurrentTime() - start;
  ...
  for (unsigned i = 0; i < row_size; i++) {
    for (unsigned j = 0; j < col_size; j++) {
      if (vec_vote_pool[i * col_size + j] > max_vote) {
        max_vote = vec_vote_pool[i * col_size + j];
        max_anchor_id = i;
        max_frame_id = j;
      }
    }
  }
  spdlog::info("res:: max:{}, anchor:{}, frame:{}, {}", max_vote, max_anchor_id,
               max_frame_id, max_anchor_id * row_size + max_frame_id);
  return profile;
}
```
