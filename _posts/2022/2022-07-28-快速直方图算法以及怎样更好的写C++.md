...
...
```
使用 u_int32_t 来代替 pair 数据结构，出发点是 pair的两个值范围不同，分别需要18位和14位表示，pair就需要更大的存储空间。
也可以用一个32位来存储，通过位运算来分别拿到两个值，相当于用时间换空间。以及降低 pair存储空间不连续造成性能的下降（实际上并不会）。

实际测试下来，发现两种情况下，性能差不多。

v3c，采用 SSE/AVX 指令完成运算，每次同时存取 4 个值，即降低之前说的计算indice的步骤，事实证明这步并不是瓶颈。
因为每个数值在数轴上是随机分布的，因此 SSE 并不会提升 vote 的速度。
