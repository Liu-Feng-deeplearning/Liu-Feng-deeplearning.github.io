...
声码器是语音领域里一个经典的生成问题，对于给定一个mel谱x，怎样生成对应的信号y，
y-》x过程非常简单，但逆问题却比较难。特别是希望能高效的求解。

另外声码器问题和图像生成问题一个显著的区别是，对应图像生成，condition一般是一个类别标签。
对于同一类别标签，显然有各种风格迥异的图片生成。但对于声码器来说，一旦给定conditon mel以后
，音频信号大体上确定了，只有相位信息以及fft高频截断信息被丢掉了。
而高频信息其实人耳听上去的主观感受并不明显。这表面，相同cond条件下，音频生成的丰富度，远低于图像生成问题。
声码器更近似与一个1vs1的问题，而不是1vsN。因此一些常见的 model-collasp/over-smooth 等生成问题中常见的 badcase
在这里表现的并不明显。
