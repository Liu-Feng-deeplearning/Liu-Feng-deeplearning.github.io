...
最近在接触
 ...
 Online Music Finder / Music Fingerprint / 实时音乐检索


## Introduction
给定一段音频作为anchor(多为bgm)，在实时音频流(target)中，检测首次出现指定音频片段的起始时间点。

challenge:  
  - 可能存在噪声干扰/信号编码损失，target 和 anchor 不一定完全对应。
  - 检测准确率，精度要求 100ms 以内，延时不超过 10s
  - 实时音频流为输入，可能存在一定概率的丢帧和乱序 (直播场景)  
  
## 算法原理

[音频指纹算法原理](doc/音频指纹算法原理.md)
...
```python
from MusicFinder import MusicFinderEngine
eng = MusicFinderEngine()
eng.add_anchor(anchor_path)
eng.search_online(target_path)
``` 
---
...
- https://arxiv.org/pdf/2010.11910.pdf
