...
其实原理上并不复杂，大体上无非就是，提取finger/搜索/排序等几个步骤，
核心是鲁棒性和计算效率的平衡。
...
<div align=center><img src="https://github.com/Liu-Feng-deeplearning/Liu-Feng-deeplearning.github.io/blob/master/images/posts/2021/freq.png?raw=true" width="600" /></div>
...
<div align=center><img src="https://github.com/Liu-Feng-deeplearning/Liu-Feng-deeplearning.github.io/blob/master/images/posts/2021/freq2.png?raw=true" width="400" /></div>
...
更新: 后来，我们发现，根据子带进行划分，可能会对子带边缘的指纹点过于敏感。例如，上例中指纹201位于子带边缘，
...
```text  
  ...
  while (j < 6) {
    ...
    int idx;
    freq.segment(start, end - start).maxCoeff(&idx); 
    idx = idx + start;
...
<div style="text-align: center"><img src="https://github.com/Liu-Feng-deeplearning/Liu-Feng-deeplearning.github.io/blob/master/images/posts/2021/voting.png?raw=true" width="400" /></div>
...
我们使用了 hash 算法来进行存储指纹和比对。对于每帧上的 6 个特征点，我们将其任意3个组成一个指纹(fp
...
- demo4: 双马尾case，anchor 为 data/demo4/双马尾FinalMMA.wav, target为 双马尾FinalFinalLive50.wav 之前拼接其他的现场片段后的音频。

说明: 
1. 两个demo可在data文件夹下找到。    
2. 看上去 demo2 比 demo3 更难一点。anchor 和 target 非同源。


为测试鲁棒性，对音频进行了加噪(Gauss)处理后，测试如下:

start point/sp: anchor 音频开始的时间 [s]      
delay time/dt: 检测到结果距离anchor开始时间的间隔 [s]

 noise[db]| demo2[sp/dt] | demo3[sp/dt] | demo4[sp/dt]
:---:|:---:|:---:|:---:
0 db | 228.9/9.2 | 319.9/5.1| -
6 db | 228.9/11.1 | 319.9/5.3| -
20 db | 228.9/10.0 | 319.9/5.1|1046.0/4.96

备注：实际对齐点位于228.9～229.0之间。
...
这里我们使用分贝值 db 衡量噪声强度。0db 表示噪声强度和人声强度相等，
10db 表示噪声强度强于人声10分贝。    
